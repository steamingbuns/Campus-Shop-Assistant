...
export async function handleChatbotQuery(req, res) {
  try {
    ...
    // Delegate full message handling to the service layer. `chatbotService.handleMessage`
    // encapsulates NLP calls, action routing (search/purchase/etc), and persistence
    // hooks. We dynamically import it to avoid module system interop issues and to
    // keep controllers thin (MVC best practice).
    const svcModule = await import('../services/chatbotService.js');
    // Support both ESM default and CommonJS exports
    const chatbotService = svcModule.default || svcModule;

    // Call the service and let it orchestrate NLP, product search, LLM calls, etc.
